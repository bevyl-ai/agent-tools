...
export function slackApiTool(name: string, token: string, description: string, fetchFn: typeof fetch = fetch): DynamicTool<z.infer<typeof Input>, string> {
  return {
    name,
    description,
    input: Input,
    async run({ method, args }) {
      ...
      if (!res.ok) throw new Error(`slack ${res.status}: ${text.slice(0, 2000)}`)
