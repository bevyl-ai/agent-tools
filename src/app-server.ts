...
export class AppServerSession {
  ...
  async runTurns(next: () => string | null, workspace = this.codex.cwd ?? '', title = this.codex.title ?? ''): Promise<void> {
    ...
    try {
      ...
      for (let prompt = next(); prompt !== null; prompt = next()) await this.runTurn(threadId, workspace, prompt, title)
    } finally {
      this.stop()
    }
  }
  ...
  private handleNotification(method: string, params: Json): void {
    ...
    if (method === 'thread/tokenUsage/updated') {
      ...
      // Codex also sends this notification with NO tokenUsage as a rate-limit-only ping — forward the limits but
      // never a fabricated all-zero count: the caller folds cumulative deltas, and a phantom zero resets its base.
      if (params.tokenUsage == null) {
        const rl = extractRateLimits(params)
        if (rl.rateLimits) this.onEvent(rl)
        return
      }
