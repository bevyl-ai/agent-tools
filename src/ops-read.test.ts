...
test('slack is read-only: posting and non-allowlisted methods are refused', () => {
  expect(JSON.stringify(resolveOpsRequest('slack', 'POST', '/api/chat.postMessage'))).toContain('refused')
  expect(JSON.stringify(resolveOpsRequest('slack', 'GET', '/api/chat.postMessage'))).toContain('refused')
  expect(JSON.stringify(resolveOpsRequest('slack', 'POST', '/api/conversations.replies'))).toContain('refused')
})
...
test('allows the read endpoints on each service', async () => {
  ...
  expect(url(resolveOpsRequest('slack', 'GET', '/api/conversations.replies?channel=C1&ts=1.2'))).toBe('https://slack.com/api/conversations.replies?channel=C1&ts=1.2')
  expect(url(resolveOpsRequest('slack', 'GET', '/api/users.info?user=U1'))).toContain('/api/users.info')
